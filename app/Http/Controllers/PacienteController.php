...
class PacienteController extends Controller
{
    ...
    {
        if (!$id_medico) return response()->json([
            'error' => 'Id do medico é obrigatório na url'
        ], 404);
        try {
            //Retorna o paciente relacionando com a tabela de vinculo de paciente e medico
            $pacientes = Paciente::join('medico_paciente', 'paciente.id', '=', 'medico_paciente.paciente_id')
                                 ...
                                 ->get();
            if (!$pacientes) return response()->json([
    ...
    public function insertPatient(Request $request)
    {
        ...
        foreach ($data as $key => $value) {
            if ($key != "nome" && $key != "celular" && $key != "cpf") return response()->json([
                'error' => 'Apenas o campo nome, celular e cpf podem ser adicionados'
            ], 422);
        }
        $pacienteExistente = Paciente::where('cpf', 'like', $data['cpf'])->first();
        if ($pacienteExistente) return response()->json([
            'error' => 'Usuario com esse CPF já existente'
        ], 409);
    ...
    public function updatePatient(Request $request, $id_paciente)
    {
        $data = $request->json()->all();
        foreach ($data as $key => $value) {
            if ($key != "nome" && $key != "celular") return response()->json([
                'error' => 'Apenas o campo nome e celular podem ser alterados'
            ], 422);
        }
        try {
            //Retorna erro caso não achar o paciente
